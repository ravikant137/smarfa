...
export default function DocumentScanScreen() {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${getApiBaseUrl()}/api/v1/scan_document`, payload, { timeout: 300000 });
  ...
  return (
    <LinearGradient colors={['#09090E', '#141421']} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <GradientHeader subtitle="AI Document Extraction" />
          ...
          <Animated.View style={[styles.glassCard, loading && { transform: [{ scale: pulseAnim }] }]}>
            {image ? (
              ...
              <View style={styles.placeholder}>
                <MaterialCommunityIcons name="file-document-outline" size={80} color="#00E676" style={styles.glowIcon} />
                <Text style={styles.placeholderText}>Upload Document</Text>
                <Text style={styles.placeholderSub}>Extract text, warnings, and usage from labels</Text>
              </View>
            )}
          </Animated.View>
          ...
          {analysis && (
            <Animated.View style={[styles.resultsContainer, { opacity: fadeAnim, transform: [{ translateY: resultsSlideAnim }] }]}>
              ...
              <View style={styles.resultHeader}>
                <View>
                  <Text style={styles.cropTitle}>{analysis.document_type || 'Document Analyzed'}</Text>
                </View>
                <View style={[styles.badge, { backgroundColor: '#2979FF1A', borderColor: '#2979FF' }]}>
                  <MaterialCommunityIcons name="file-check" size={18} color="#2979FF" />
                  <Text style={[styles.badgeText, { color: '#2979FF' }]}>EXTRACTED</Text>
                </View>
              </View>

              <View style={styles.cardSection}>
                <Text style={styles.sectionHeading}><MaterialCommunityIcons name="text-box-search" size={18} /> Extracted Information</Text>
                <Text style={styles.sectionBody}>{analysis.extracted_text}</Text>
              </View>

              {analysis.usage_instructions && (
                <View style={styles.cardSection}>
                  <Text style={[styles.sectionHeading, { color: '#00E676' }]}><MaterialCommunityIcons name="information" size={18} /> Usage Instructions</Text>
                  <Text style={styles.sectionBody}>{analysis.usage_instructions}</Text>
                </View>
              )}

              {analysis.warnings && (
                <View style={styles.cardSection}>
                  <Text style={[styles.sectionHeading, { color: '#FF4B4B' }]}><MaterialCommunityIcons name="alert" size={18} /> Warnings</Text>
                  <Text style={styles.sectionBody}>{analysis.warnings}</Text>
                </View>
              )}

              {analysis.active_ingredients && (
                <View style={styles.cardSection}>
                  <Text style={[styles.sectionHeading, { color: '#FFB800' }]}><MaterialCommunityIcons name="flask" size={18} /> Active Ingredients</Text>
                  <Text style={styles.sectionBody}>{analysis.active_ingredients}</Text>
                </View>
              )}
